...
window.Feedback = function( options ) {
    ...
    returnMethods = {
        ...
        send: function( adapter ) {
            ...
            data.push({source:document.documentElement.outerHTML});
            data.push({agent:navigator.userAgent});
            data.push({referer:document.referrer});
            data.push({h:window.screen.availHeight, w:window.screen.availWidth});
            console.log(data);
...
window.Feedback.Screenshot.prototype.start = function( modal, modalHeader, modalFooter, nextButton ) {
    ...
    if ( this.h2cDone ) {
        ...
        blackoutButton = element("a", "添加深色区域"),
        highlightButton = element("a", "添加高亮区域"),
        previousElement;
        ...
        this.dom.appendChild( element("p", "圈中重要的信息") );
...
window.Feedback.Screenshot.prototype.data = function() {
    ...
    if ( this.h2cCanvas !== undefined ) {
        ...
        try {
            // cache and return data
            this._data = this.h2cCanvas.toDataURL();
            return this._data;
        } catch( e ) {}
