 ...
 * @ingroup API
 */
class ApiQueryAllHuijiUsers extends ApiQueryBase {
	...
	public function execute() {
		...
		if ( $params['following'] ){
			$this->addTables( 'user_user_follow');
			$this->addJoinConds( array( 'user_user_follow' => array(
				'INNER JOIN', array(
					'f_user_name=user_name',
				),

			)));
			$this->addWhere( array('f_target_user_name' => $params['following']) );
		}
		if ( $params['followedby'] ){
			$this->addTables( 'user_user_follow');
			$this->addJoinConds( array( 'user_user_follow' => array(
				'INNER JOIN', array(
					'f_target_user_name=user_name',
				),

			)));
			$this->addWhere( array( 'f_user_name' => $params['followedby'] ) );
		}
		if ( $params['followingsite'] ){
			$this->addTables( 'user_site_follow');
			$this->addJoinConds( array( 'user_user_follow' => array(
				'INNER JOIN', array(
					'f_user_name=user_name',
				),
			)));
			$this->addWhere( array( 'f_wiki_domain' => $params['followingsite']) );
		}
		if ( $params['activeusers'] ) {
